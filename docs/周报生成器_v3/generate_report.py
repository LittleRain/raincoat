...
def classify_vec(df, up_ids=None, has_b1=True, has_b3=False, has_mid=False):
    ...
    if up_ids and has_mid:
        mid_col = '商家ID' if '商家ID' in df.columns else ('商户id' if '商户id' in df.columns else None)
        if mid_col:
            def _chk(x):
                try: return pd.notna(x) and int(float(str(x))) in up_ids
                except: return False
            is_up = df[mid_col].apply(_chk)
            m = is_up & (ind == '其他')
...
def load_all(dd):
    ...
    for k, v in files.items():
        if k == 'UP' and v:
            df_up = read(v)
            # 自动检测表头行（可能在第0行或已是列名）
            if '商家ID' in df_up.columns:
                pass  # 列名已正确
            elif len(df_up) > 0:
                # 查找包含'商家ID'的行作为表头
                for ri in range(min(3, len(df_up))):
                    row_vals = [str(x) for x in df_up.iloc[ri].tolist()]
                    if any('商家ID' in v2 for v2 in row_vals):
                        cols = df_up.iloc[ri].tolist()
                        df_up = df_up.iloc[ri + 1:].reset_index(drop=True)
                        df_up.columns = cols
                        break
            if '商家ID' in df_up.columns:
                dfs['up_ids'] = set(int(float(x)) for x in df_up['商家ID'].dropna())
                print(f"  → UP主ID: {len(dfs['up_ids'])}个")
            else:
                print(f"  ⚠ UP文件中未找到'商家ID'列: {list(df_up.columns)}")
                dfs['up_ids'] = set()
        elif v:
            dfs[k] = read(v)
