...
处方按「能不能替你做决定」分三级。**这三档描述的是方案包脚本的执行方式，不是页面上的按钮** —— 去重要把一个真实目录换成快捷方式，属于写操作，统一走「生成处理方案包 → 先看干跑 → `DRY_RUN=0` 执行」，页面不提供一键：

| 级别 | 判定条件 | 脚本行为 |
|---|---|---|
| **脚本可自动** | 正文逐字一致（`identical`）等元数据级差异（`meta-only`） | 直接换：留一份作正本，其余改成指向它的快捷方式 |
...
正本挑选的评分顺序：跨 agent 共享池（天然被多方引用）> 不在 App 包内（改它不会被升级覆盖）> 已被其他快捷方式引用 > 实体本体。评分并列时按路径长度兜底 —— 这个兜底是任意的，**并列时会挑到别的 agent 的副本**（例：`~/.workbuddy/skills/eli5` 与 `~/.codex/skills/eli5` 逐字一致，正本会选路径更短的那份）。目前 `overrides.json` **没有**指定正本的字段，想改只能手动改脚本或先删掉不想要的那份再重扫。

方案包产出两份东西：`plan-<时间戳>.md`（给人看的清单：留哪份、为什么、会影响谁）和 `plan-<时间戳>.sh`（给机器跑的脚本，默认 `DRY_RUN=1` 什么都不改）。页面里给出的可复制命令是 `cd <skill>/scripts && python3 skillctl.py …`（入口目录由 `ENTRY_DIR` 注入，有回归测试守着能真跑）；本文件与 `SKILL.md` 的命令从 skill 根写起，两者等价。
