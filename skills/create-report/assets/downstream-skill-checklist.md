...
- `README.md` 自描述，不要求用户再去读取仓库外部需求文档才能运行
- `README.md` 已写明必需输入文件、pattern、字段和输出产物
...
- `skill.json` 的 `visibility` 不是 `internal`
- `skill.json` 的 `status` 与对外发布阶段一致
- 全包不依赖上游 `create-report`、`report-creator` 或原始需求文档路径
- 输入合同已经内嵌到 README、SKILL 或 examples 中，包本身可独立理解
